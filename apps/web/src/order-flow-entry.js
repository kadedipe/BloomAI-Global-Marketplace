...
function openCheckout(product){
  ...
  backdrop.innerHTML=`<section class="order-dialog" role="dialog" aria-modal="true"><button class="order-close" aria-label="Close">×</button><span class="order-kicker">Secure checkout</span><h2>Order ${escapeHtml(product.name)}</h2><p>Complete delivery details before continuing to Paystack.</p><form class="order-form"><label>Quantity<input name="quantity" type="number" min="1" max="20" value="1" required></label><label>Recipient name<input name="recipient_name" value="${escapeAttr(user.name||'')}" minlength="2" maxlength="120" required></label><label>Phone<input name="phone" minlength="5" maxlength="40" required></label><label class="wide">Delivery address<input name="address_line1" minlength="4" maxlength="240" required></label><label>City<input name="city" minlength="2" maxlength="120" required></label><label>Region / State<input name="region" maxlength="120"></label><label>Postal code<input name="postal_code" maxlength="32"></label><label>Country<input name="country" minlength="2" maxlength="120" required></label><label class="wide">Order note<textarea name="buyer_note" maxlength="1000" rows="3" placeholder="Optional delivery instructions"></textarea></label><div class="order-total wide">Total <strong>${money(product.currency,product.price)}</strong></div><div class="order-status wide" role="status"></div><div class="order-actions wide"><button type="button" class="secondary order-cancel">Cancel</button><button class="primary order-submit">Continue to payment</button></div></form></section>`;
  ...
  const quantity=backdrop.querySelector('[name=quantity]');const total=backdrop.querySelector('.order-total strong');quantity.oninput=()=>{total.textContent=money(product.currency,Number(product.price)*Math.max(1,Number(quantity.value)||1))};
  backdrop.querySelector('form').onsubmit=async event=>{event.preventDefault();const status=backdrop.querySelector('.order-status');const submit=backdrop.querySelector('.order-submit');submit.disabled=true;status.textContent='Creating your order…';const data=Object.fromEntries(new FormData(event.currentTarget));data.product_id=product.id;data.quantity=Number(data.quantity);try{const response=await api('/api/v1/orders/checkout',{method:'POST',body:JSON.stringify(data)});const body=await response.json();if(!response.ok)throw new Error(body.detail||'Order could not be created.');window.location.assign(body.authorization_url)}catch(error){status.textContent=error.message;status.classList.add('error');submit.disabled=false}};
}
...
async function renderOrders(){
  ...
  section.querySelectorAll('[data-cancel]').forEach(button=>button.onclick=async()=>{await api(`/api/v1/orders/${button.dataset.cancel}/cancel`,{method:'PATCH'});renderOrders()});
